...
function formatResult(result, json) {
    if (json) {
        return JSON.stringify(result, (_, v) => typeof v === 'bigint' ? v.toString() + 'n' : v);
    }
...
export async function runCommand(file, func, args, options) {
    ...
    try {
        ...
        if (debug) {
            console.log(chalk.dim(`Calling: ${functionName}(${parsedArgs.map(a => typeof a === 'bigint' ? `${a}n` : JSON.stringify(a)).join(', ')})`));
        }
        // Call function
