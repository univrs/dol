 ...
 */
async function parseWasmModule(wasmBytes) {
    const bytes = new Uint8Array(wasmBytes).buffer;
    const module = await WebAssembly.compile(bytes);
    // Get exports
    const exports = WebAssembly.Module.exports(module).map((exp) => ({
        name: exp.name,
        kind: exp.kind,
    }));
    // Get imports
...
export async function inspectCommand(file, options) {
    try {
        // Resolve file path
        ...
        if (options.json) {
            ...
            if (options.imports || (!options.exports && !options.memory))
                output.imports = info.imports;
            if (options.memory || (!options.exports && !options.imports))
                output.memory = info.memory;
